...
import { Schema, Table, Column, PostgreSqlColumnSchema } from '../commons/types';
 ...
 */
export default class PostgresqlSchemaPreprocessor {
  ...
  public convertToStandardSchema(columnSchema: PostgreSqlColumnSchema): Column {
    ...
    const column: Column = {
      ...
      primary: Boolean(columnSchema.column_key && columnSchema.column_key === 'PRIMARY KEY'),
      unique: Boolean(
        columnSchema.column_key && (columnSchema.column_key === 'PRIMARY KEY' || columnSchema.column_key === 'UNIQUE'),
      ),
